...
const HeaderBlack = () => {
  ...
  return (
    <div className="lg:h-20 fixed top-0 left-0 w-full z-50 h-[3.5rem] bg-lip_white opacity-95">
      <div className="lg:px-[6.25rem] md:px-8 px-5 text-lip_menu flex h-full items-center">
        {/* 로고 */}
        ...
        {/* 네비게이션 메뉴 */}
        <div
          className={`lg:flex md:flex lg:justify-center md:justify-center absolute md:static lg:static top-[3.5rem] left-0 w-full bg-lip_white lg:bg-transparent md:bg-transparent transition-all duration-500 ease-in-out overflow-hidden ${
            isOpen
              ? "max-h-[300px] pb-16 opacity-100"
              : "max-h-0 pb-0 opacity-0"
          } lg:max-h-none md:max-h-none lg:opacity-100 md:opacity-100 lg:pb-0 md:pb-0`}
        >
          <ul className="lg:flex md:flex lg:space-x-6 md:space-x-6 py-6 flex flex-col lg:flex-row md:flex-row items-center space-y-4 lg:space-y-0 md:space-y-0 p-0 m-0 text-lip_menu">
            <li>
              <NavLink
                onClick={() => setIsOpen(!isOpen)}
                to="/support"
        ...
        >
          <button className="lg:w-32 md:w-32 px-5 py-[0.375rem] text-lip_white bg-lip_purple hover:bg-lip_purple_hover rounded-3xl font-bold flex items-center justify-center whitespace-nowrap">
            {token ? (
